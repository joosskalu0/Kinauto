  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-sans antialiased">
      ...
      <Header
        ...
        openDealershipAuthModal={() => setIsDealershipAuthModalOpen(true)}
        onLogoutDealership={handleLogoutDealership}
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        {currentView === 'public' && (
          <div className="space-y-8">
            ...
            {/* Kinshasa SOS Breakdown / Garage Directory Quick Banner */}
            <div className="bg-gradient-to-r from-rose-50 via-amber-50/50 to-rose-50 border border-rose-200 rounded-3xl p-6 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-amber-700 font-bold">24 Communes couvertes</span>
                  </div>
                  <h3 className="text-base font-black text-slate-900 mt-0.5">
                    Une panne, batterie à plat ou besoin d'un mécanicien à Kinshasa ?
                  </h3>
                  <p className="text-xs text-slate-600">
                    Trouvez un garage certifié ou déclenchez une intervention mobile d'urgence 24/7 (Gombe, Limete, Ngaliema, Matete...).
                  </p>
                </div>
              </div>
              <button
              ...
              >
                <span>Trouver un Dépanneur / Garage</span>
                <span>→</span>
              </button>
            </div>

            {/* Hero / Banner Section */}
            <div className="relative rounded-3xl overflow-hidden bg-slate-900 border border-slate-800 p-8 sm:p-12 shadow-md">
              <div 
                className="absolute inset-0 bg-cover bg-center opacity-40"
                style={{ backgroundImage: `url(${dealership.bannerUrl})` }}
              ></div>
              <div className="absolute inset-0 bg-gradient-to-r from-slate-950 via-slate-900/90 to-transparent"></div>

              <div className="relative max-w-2xl space-y-4">
                <div className="flex items-center gap-2">
                  <span className="bg-amber-500 text-slate-950 font-black text-[10px] uppercase px-2.5 py-1 rounded-full tracking-wider">
                    Catalogue Réseau Multi-Concessions
                  </span>
                  <span className="text-xs text-slate-300 font-medium">
                    {vehicles.filter(v => v.status === 'disponible').length} véhicules disponibles à la vente
                  </span>
                </div>

                <h2 className="text-3xl sm:text-5xl font-black text-white tracking-tight leading-tight">
                  Trouvez votre véhicule d'exception
                </h2>
                <p className="text-sm sm:text-base text-slate-200">
                  Découvrez l'ensemble des véhicules d'occasion certifiés et neufs disponibles sur notre réseau de concessions partenaires. Tous nos véhicules sont vendus avec garantie concessionnaire et possibilité de financement sur-mesure.
                </p>

                {/* Quick Brand Badges */}
                <div className="pt-2 flex flex-wrap gap-2">
                  <span className="text-xs text-slate-300 font-semibold self-center mr-1">Marques :</span>
                  <button
                ...
                {/* Dealership Network Quick Filter Pills */}
                <div className="pt-2 flex flex-wrap items-center gap-2">
                  <span className="text-xs text-slate-300 font-semibold self-center mr-1">🏢 Concessions :</span>
                  <button
                    onClick={() => setFilterDealership('ALL')}
                  ...
                  >
                    <span>🌟 Tout le réseau</span>
                    <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-black ${filterDealership === 'ALL' ? 'bg-slate-950 text-amber-400' : 'bg-white/20 text-white'}`}>
                      {vehicles.length}
                    </span>
                  </button>
                  ...
                  {visibleDealershipAccounts.map((acc) => {
                    ...
                    const isSelected = filterDealership === acc.id;
                    return (
                      <button
                        key={acc.id}
                        onClick={() => setFilterDealership(acc.id)}
                        className={`text-xs font-bold px-3 py-1.5 rounded-xl border transition cursor-pointer flex items-center gap-1.5 ${
                          isSelected
                            ? 'bg-amber-500 border-amber-500 text-slate-950 shadow-xs'
                            : 'bg-white/15 backdrop-blur-sm border-white/20 text-white hover:bg-white/25'
                      ...
                      >
                        <span>📍 {acc.info.nom} ({acc.info.ville})</span>
                        <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-black ${isSelected ? 'bg-slate-950 text-amber-400' : 'bg-white/20 text-white'}`}>
                          {count}
                        </span>
                      </button>
                    );
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-3">
                <div className="flex items-center gap-2 text-slate-900 font-bold text-sm">
                  ...
                  <span>Recherche & Filtres Avancés</span>
                </div>
                ...
                <div className="flex items-center gap-3 text-xs">
                  ...
                  <button
                    ...
                    className="text-slate-500 hover:text-amber-600 font-medium flex items-center gap-1 cursor-pointer"
                  >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-8 gap-3 text-xs">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-50 border border-slate-200 text-amber-800 font-bold rounded-xl p-2 focus:outline-none focus:border-amber-500"
                  >
                ...
                {/* Promotions */}
                <div>
                  <label className="block text-slate-600 font-medium mb-1">Offres & Promos</label>
                  <select
                    value={filterPromo}
                    onChange={(e) => setFilterPromo(e.target.value as 'ALL' | 'PROMO')}
                    className="w-full bg-slate-50 border border-slate-200 text-rose-700 font-bold rounded-xl p-2 focus:outline-none focus:border-rose-500"
                  >
                    <option value="ALL">Tous les véhicules</option>
                    <option value="PROMO">🏷️ Promos & Flash</option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-xl p-2 focus:outline-none focus:border-amber-500 font-medium"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-50 border border-slate-200 text-amber-800 font-bold rounded-xl p-2 focus:outline-none"
                  >
                    <option value="recent">Date d'ajout récents</option>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 border-t border-slate-100 text-xs">
                <div>
                  <div className="flex justify-between font-semibold text-slate-700 mb-1">
                    ...
                    <span className="text-amber-700 font-bold">{priceMax >= 350000 ? 'Illimité (350 000+ €)' : `${priceMax.toLocaleString('fr-FR')} €`}</span>
                  </div>
                  <input
                    type="range"
                    min="10000"
                ...
                <div>
                  <div className="flex justify-between font-semibold text-slate-700 mb-1">
                    ...
                    <span className="text-amber-700 font-bold">{kmMax >= 250000 ? 'Illimité (250 000+ km)' : `${kmMax.toLocaleString('fr-FR')} km`}</span>
                  </div>
                  <input
                    type="range"
                    min="5000"
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 text-xs font-semibold text-slate-600">
              <p>
                Affichage de <span className="text-amber-800 font-bold">{publicFilteredVehicles.length}</span> véhicule(s) disponible(s) dans toutes nos {dealershipAccounts.length} concessions partenaires
              </p>
              <div className="flex items-center gap-2.5">
                ...
                <div className="flex items-center bg-slate-100 border border-slate-200 p-0.5 rounded-xl">
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-lg text-xs font-bold transition flex items-center gap-1 cursor-pointer ${
                      layoutMode === 'grid' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-500 hover:text-slate-900'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-lg text-xs font-bold transition flex items-center gap-1 cursor-pointer ${
                      layoutMode === 'list' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-500 hover:text-slate-900'
                    }`}
                  >
                ...
                <button
                  ...
                  className="bg-white hover:bg-slate-50 text-amber-800 hover:text-amber-900 border border-slate-200 font-bold px-3 py-1.5 rounded-xl flex items-center gap-1.5 transition cursor-pointer shadow-xs"
                  title="Partager le stock de la concession sur les réseaux sociaux"
                >
                ...
                {isAdmin && (
                  ...
                  >
                    <PlusCircle className="w-4 h-4 text-amber-600" /> Publier
                  </button>
                )}
              </div>
            </div>

            {/* Vehicle Showcase Cards (Grid / List Layout) */}
            {publicFilteredVehicles.length === 0 ? (
              <div className="bg-white border border-slate-200 rounded-3xl p-12 text-center space-y-4 shadow-xs">
                ...
                <button
                  onClick={resetFilterOptions}
                  className="bg-amber-500 text-slate-950 font-black px-4 py-2.5 rounded-xl text-xs shadow-xs cursor-pointer hover:bg-amber-400 transition"
                >
                  Réinitialiser tous les filtres
                </button>
              </div>
            ) : (
              <div className={layoutMode === 'list' ? 'flex flex-col space-y-4' : 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6'}>
      ...
      <Footer
        ...
        isSuperAdminAuthenticated={isSuperAdminAuthenticated}
        currentAccount={currentAccount}
        onOpenDealershipLogin={() => setIsDealershipAuthModalOpen(true)}
        onOpenSuperAdminLogin={() => setIsSuperAdminAuthModalOpen(true)}
        onOpenRegisterDealership={() => setIsRegisterModalOpen(true)}
        onOpenGarages={() => setCurrentView('garages')}
        onOpenAdminDashboard={() => {
          setIsAdmin(true);
          setCurrentView(isSuperAdminAuthenticated ? 'super-admin' : 'admin-dashboard');
        }}
      />
